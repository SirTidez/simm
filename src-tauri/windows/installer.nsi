...
Var PrereqAllVerified
Var PrereqCacheDir
Var PrereqConsole
Var PrereqLogBuffer
Var PrereqConsoleFont
...
; 7. Prerequisite overview page
Page custom PagePrerequisitesIntro

; 8. Prerequisite check page
Page custom PagePrerequisites PageLeavePrerequisites
...
Function .onInit
  ...
  StrCpy $PrereqAllVerified 0
  StrCpy $PrereqCacheDir "${PREREQ_CACHE_DIR}"
FunctionEnd
...
Section Install
  ...
  ${If} $PrereqAllVerified <> 1
    Abort "Required prerequisites were not installed successfully."
  ${EndIf}
...
Function PagePrerequisitesIntro
  ${If} $PassiveMode = 1
    Abort
  ${EndIf}

  !insertmacro MUI_HEADER_TEXT "Prepare SIMM" "Setup will validate and install the Windows dependencies required for Schedule I modding."
...
Function PagePrerequisites
  ...
  !insertmacro MUI_HEADER_TEXT "Dependency Check" "Setup is validating the Windows runtimes and tools required before SIMM can install."
  ...
  ${EndIf}
  SendMessage $PrereqConsole ${WM_SETFONT} $PrereqConsoleFont 1

  GetDlgItem $PrereqNextButton $HWNDPARENT 1
  EnableWindow $PrereqNextButton 0

  GetFunctionAddress $0 BeginPrerequisiteCheck
  nsDialogs::CreateTimer $0 150

  nsDialogs::Show
FunctionEnd

Function BeginPrerequisiteCheck
  GetFunctionAddress $0 BeginPrerequisiteCheck
  nsDialogs::KillTimer $0
  Call EnsurePrerequisites
FunctionEnd

Function PageLeavePrerequisites
  ${If} $PrereqAllVerified <> 1
    MessageBox MB_ICONEXCLAMATION "SIMM cannot continue until all required prerequisites are installed and verified."
    Abort
  ${EndIf}
FunctionEnd
...
Function SetSummaryText
  Exch $0
  ${If} $PrereqSummaryLabel != ""
    ${NSD_SetText} $PrereqSummaryLabel "$0"
  ${EndIf}
  DetailPrint "$0"
  Push "$0"
...
Function ResetPrereqConsole
  StrCpy $PrereqLogBuffer ""
  ${If} $PrereqConsole != ""
    ${NSD_SetText} $PrereqConsole ""
  ${EndIf}
FunctionEnd

Function AppendPrereqLog
  Exch $0
  Push $1
  DetailPrint "$0"
  ${If} $PrereqLogBuffer == ""
    StrCpy $PrereqLogBuffer "$0"
    StrCpy $1 "$0$\r$\n"
  ${Else}
    StrCpy $PrereqLogBuffer "$PrereqLogBuffer$\r$\n$0"
    StrCpy $1 "$0$\r$\n"
  ${EndIf}
  ${If} $PrereqConsole != ""
    SendMessage $PrereqConsole ${EM_SETSEL} -1 -1
    SendMessage $PrereqConsole ${EM_REPLACESEL} 0 "STR:$1"
    SendMessage $PrereqConsole ${EM_SETSEL} -1 -1
    SendMessage $PrereqConsole ${EM_SCROLLCARET} 0 0
  ${EndIf}
  Call FlushInstallerWindow
  Pop $1
  Pop $0
...
Function SetVCRedistText
  Exch $0
  ${If} $PrereqVCRedistLabel != ""
    ${NSD_SetText} $PrereqVCRedistLabel "$0"
  ${EndIf}
  Call FlushInstallerWindow
  Pop $0
FunctionEnd
...
Function SetDepotDownloaderText
  ...
  ${If} $PrereqDepotDownloaderLabel != ""
    ${NSD_SetText} $PrereqDepotDownloaderLabel "$0"
  ${EndIf}
  Call FlushInstallerWindow
  Pop $0
FunctionEnd
...
Function EnsurePrerequisites
  StrCpy $PrereqAllVerified 0
  CreateDirectory $PrereqCacheDir
  Call ResetPrereqConsole

  ${If} $PrereqRetryButton != ""
    EnableWindow $PrereqRetryButton 0
  ${EndIf}
  ${If} $PrereqNextButton != ""
    EnableWindow $PrereqNextButton 0
  ${EndIf}

  Push "Checking required Windows components..."
  Call SetSummaryText
  ...
  Call EnsureDepotDownloader
  Pop $0
  ${If} $0 <> 1
    Push "DepotDownloader is still missing. Install it manually or fix winget, then retry."
    Call SetSummaryText
    Goto prereq_done
  ${EndIf}

  StrCpy $PrereqAllVerified 1
  Push "System check complete. Continue to install SIMM."
  Call SetSummaryText

  prereq_done:
  ${If} $PrereqRetryButton != ""
    EnableWindow $PrereqRetryButton 1
  ${EndIf}
  ${If} $PrereqNextButton != ""
    EnableWindow $PrereqNextButton $PrereqAllVerified
  ${EndIf}
FunctionEnd

Function EnsureVCRedist
  Push "Checking VC++ runtime registry and uninstall entries..."
  Call AppendPrereqLog
  Call DetectVCRedist
  Pop $0
  ${If} $0 = 1
    Push "VC++ Runtime: Detected"
    Call SetVCRedistText
    Push "VC++ runtime detection succeeded."
    Call AppendPrereqLog
    Push 1
    Return
  ${EndIf}

  Push "VC++ Runtime: Downloading"
  Call SetVCRedistText
  Push "Downloading ${PREREQ_VCREDIST_URL}"
  Call AppendPrereqLog
  NSISdl::download "${PREREQ_VCREDIST_URL}" "$PrereqCacheDir\${PREREQ_VCREDIST_FILENAME}"
  Pop $0
  ${If} $0 != "success"
    Push "Failed to download ${PREREQ_VCREDIST_NAME}: $0"
    Call AppendPrereqLog
    Push "VC++ Runtime: Failed"
    Call SetVCRedistText
    Push 0
    Return
  ${EndIf}
  ...
  Call AppendPrereqLog
  ExecWait '"$PrereqCacheDir\${PREREQ_VCREDIST_FILENAME}" ${PREREQ_VCREDIST_ARGS}' $0
  ${If} $0 <> 0
  ${AndIf} $0 <> 3010
    Push "${PREREQ_VCREDIST_NAME} installer exited with code $0"
    Call AppendPrereqLog
    Push "VC++ Runtime: Failed"
    Call SetVCRedistText
    Push 0
    Return
  ${EndIf}
  Push "VC++ installer exit code: $0"
  Call AppendPrereqLog

  Push "Re-checking VC++ runtime after install..."
  Call AppendPrereqLog
  Call DetectVCRedist
  Pop $0
  ${If} $0 = 1
    Push "VC++ Runtime: Verified"
    Call SetVCRedistText
    Push "VC++ runtime verification succeeded."
    Call AppendPrereqLog
    Push 1
    Return
  ${EndIf}

  Push "${PREREQ_VCREDIST_NAME} installer finished but verification failed."
  Call AppendPrereqLog
  Push "VC++ Runtime: Failed"
  Call SetVCRedistText
  Push 0
...
Function EnsureDepotDownloader
  Push "Checking DepotDownloader installation..."
  Call AppendPrereqLog
  Call DetectDepotDownloader
  Pop $0
  ${If} $0 = 1
    Push "DepotDownloader CLI: Detected"
    Call SetDepotDownloaderText
    Push "DepotDownloader detection succeeded."
    Call AppendPrereqLog
    Push 1
    Return
  ${EndIf}
  ...
  Pop $1
  ${If} $1 == ""
    Push "winget was not found. Manual install required: winget install --exact --id ${PREREQ_DEPOTDOWNLOADER_WINGET_ID}"
    Call AppendPrereqLog
    ${If} $PassiveMode != 1
      MessageBox MB_ICONEXCLAMATION "winget is not available on this system. Install DepotDownloader manually with:$\r$\n$\r$\nwinget install --exact --id ${PREREQ_DEPOTDOWNLOADER_WINGET_ID}$\r$\n$\r$\nThen click Retry."
    ${EndIf}
    Push "DepotDownloader CLI: Failed"
    Call SetDepotDownloaderText
    Push 0
    Return
  ${EndIf}

  Push "DepotDownloader CLI: Installing"
  Call SetDepotDownloaderText
  Push "Running winget via: $1 ${PREREQ_DEPOTDOWNLOADER_WINGET_ARGS}"
  Call AppendPrereqLog
  nsExec::ExecToStack '$1 ${PREREQ_DEPOTDOWNLOADER_WINGET_ARGS}'
  Pop $0
  Pop $2
  Push "winget output: $2"
  Call AppendPrereqLog
  ${If} $0 <> 0
    Push "winget install failed with code $0"
    Call AppendPrereqLog
    ${If} $PassiveMode != 1
      MessageBox MB_ICONEXCLAMATION "winget could not install DepotDownloader automatically.$\r$\n$\r$\nOutput:$\r$\n$2$\r$\n$\r$\nInstall DepotDownloader manually, then click Retry."
    ${EndIf}
    Push "DepotDownloader CLI: Failed"
    Call SetDepotDownloaderText
    Push 0
  ...
  Push "Re-checking DepotDownloader after install..."
  Call AppendPrereqLog
  Call DetectDepotDownloader
  Pop $0
  ${If} $0 = 1
    Push "DepotDownloader CLI: Verified"
    Call SetDepotDownloaderText
    Push "DepotDownloader verification succeeded."
    Call AppendPrereqLog
    Push 1
    Return
  ${EndIf}

  Push "DepotDownloader installation completed but verification failed."
  Call AppendPrereqLog
  Push "DepotDownloader CLI: Failed"
  Call SetDepotDownloaderText
  Push 0
...
Function DetectVCRedist
  ...
  ${If} $0 = 1
    Push "VC++ registry key HKLM\SOFTWARE\Microsoft\VisualStudio\14.0\VC\Runtimes\x64 reports Installed=1"
    Call AppendPrereqLog
    StrCpy $3 1
    Goto detect_vc_done
  ${EndIf}
  Push "VC++ runtime registry key not satisfied, falling back to uninstall entry scan."
  Call AppendPrereqLog
  StrCpy $0 0
  detect_vc_loop:
    EnumRegKey $1 HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall" $0
    StrCmp $1 "" detect_vc_done
    IntOp $0 $0 + 1
    ReadRegStr $2 HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\$1" "DisplayName"
    ${StrLoc} $R0 $2 "Microsoft Visual C++ 2015" ">"
    ${If} $R0 == ""
      Goto detect_vc_loop
    ${EndIf}
    ${StrLoc} $R1 $2 "(x64)" ">"
    ${If} $R1 == ""
      Goto detect_vc_loop
    ${EndIf}
    Push "Matched VC++ uninstall entry: $2"
    Call AppendPrereqLog
    StrCpy $3 1
  detect_vc_done:
  ${If} $3 = 0
...
Function DetectDepotDownloader
  StrCpy $2 0

  nsExec::ExecToStack '"$SYSDIR\cmd.exe" /C where DepotDownloader.exe'
  Pop $0
  Pop $1
  Push "where DepotDownloader.exe exit code: $0"
  Call AppendPrereqLog
  ${If} $1 != ""
    Push "where DepotDownloader.exe output: $1"
    Call AppendPrereqLog
  ${EndIf}
  ${If} $0 == 0
  ${AndIf} $1 != ""
    StrCpy $2 1
    Goto detect_depot_done
  ${EndIf}

  IfFileExists "$LOCALAPPDATA\Microsoft\WinGet\Packages\SteamRE.DepotDownloader_Microsoft.Winget.Source_8wekyb3d8bbwe\DepotDownloader.exe" 0 +3
    Push "Matched DepotDownloader in WinGet package directory."
    Call AppendPrereqLog
    StrCpy $2 1
    Goto detect_depot_done

  IfFileExists "$PROGRAMFILES\DepotDownloader\DepotDownloader.exe" 0 detect_depot_done
  Push "Matched DepotDownloader in Program Files."
  Call AppendPrereqLog
  StrCpy $2 1

  detect_depot_done:
  ${If} $2 = 0
    Push "DepotDownloader was not detected."
    Call AppendPrereqLog
  ${EndIf}
  Push $2
FunctionEnd
...
Function GetWingetPath
  IfFileExists "$LOCALAPPDATA\Microsoft\WindowsApps\winget.exe" 0 +3
    StrCpy $0 '"$LOCALAPPDATA\Microsoft\WindowsApps\winget.exe"'
    Goto get_winget_done

  nsExec::ExecToStack '"$SYSDIR\cmd.exe" /C where winget.exe'
  Pop $1
  Pop $0
  ${If} $1 == 0
  ${AndIf} $0 != ""
...
Function FirstLine
  ...
  first_line_loop:
    StrCpy $1 $0 1 $2
    StrCmp $1 "" first_line_done
    StrCmp $1 "$\r" first_line_done
    StrCmp $1 "$\n" first_line_done
    IntOp $2 $2 + 1
    Goto first_line_loop
  first_line_done:
  StrCpy $0 $0 $2
  Exch $0
FunctionEnd
