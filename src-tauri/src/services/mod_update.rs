...
impl ModUpdateService {
    ...
    pub async fn check_library_mod_updates(
        ...
        let mut processed_storage_ids = HashSet::new();
        let now = Utc::now();
        ...
        for entry in &library.downloaded {
            ...
            for (runtime_label, storage_id) in runtime_storage_pairs {
                if storage_id.trim().is_empty() || !processed_storage_ids.insert(storage_id.clone())
                {
                    continue;
                }
                ...
                let metadata = ModMetadata {
                    ...
                    library_added_at: storage_metadata.library_added_at.or(entry.library_added_at),
                    ...
                    update_available: storage_metadata.update_available.or(entry.update_available),
