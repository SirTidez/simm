...
static FOMOD_SERVICE: Lazy<Arc<Mutex<FomodService>>> = Lazy::new(|| {
    Arc::new(Mutex::new(FomodService::new()))
});
...
pub async fn detect_fomod(zip_path: String) -> Result<serde_json::Value, String> {
    ...
    let result = service.detect_fomod(Path::new(&zip_path))
...
pub async fn parse_fomod_xml(zip_path: String) -> Result<serde_json::Value, String> {
    ...
    let config = service.parse_fomod_xml(Path::new(&zip_path))
        .map_err(|e| e.to_string())?;
