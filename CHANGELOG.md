...
- Routed custom Schedule I installs through Steam shortcuts so explicit Steam launches stay Steam-owned, while preserving clearer retry/restart handling when Steam needs to reload shortcut configuration.
- Improved wizard configuration spacing and dialog colors so setup forms, directory rows, and related disabled states read more clearly.
- Reduced duplicate concurrent frontend/backend refresh work with in-flight request coalescing and short-lived mod-library caching, and added startup timing instrumentation to make app boot behavior easier to measure.
- Expanded environment profile sharing and import flows so profiles can be imported, resolved automatically, detected by target files, and previewed with clear count pills; follow-up fixes keep the sharing UI stable after review feedback.
- Promoted Profiles into a first-class sidebar workspace with IL2CPP and Mono profile browsing, clearer default profile cards, compatible target selection, Apply, Apply & Launch, JSON import/export, rename/delete, and header-level visibility into which environments use the selected profile.
- Expanded profile library behavior with runtime-locked default profiles, enabled/disabled item state, create-from-selected-target-items and capture-snapshot flows, exact-sync apply behavior, active-profile tracking, and safer delete guards for profiles currently in use.
- Aligned the 0.8.6 app/package metadata and added pre-publish validation gates so release artifacts are built only after frontend and Rust checks pass.
- Reduced development rebuild noise by ignoring Rust target output in Vite's watcher stream.
- Added async startup flow with boot UI updates so initial application load provides clearer progress and avoids blocking behaviors during initialization.
- Improved the Logs viewer by combining multi-line Il2Cpp/MelonLoader exception entries, preserving backend warning/error severity, and tagging generic Il2Cpp errors with likely installed mod names when matching mod DLLs are present.
- Stabilized live-log bottom scrolling and large multi-line entry virtualization so Jump to Live and tailed log browsing no longer jitter or misplace the viewport around expanded stack traces.
- Added Linux baseline parity updates, including containerized Linux build tooling and docs/workflow validation so release prep and cross-platform runtime coverage can be exercised consistently.
- Improved launch reliability with Steam shortcut reload marker tracking plus environment-style and download-directory helper updates, and tightened Steam auth/UI behavior around desktop launch workflows.
- Added opt-in live telemetry that tracks registered running environments, tails game logs for local warning/error events, keeps a durable local event ledger, supports privacy/retention controls, and produces a versioned local export preview without adding client upload behavior.
- Added a tray-resident desktop runtime with configured background update checks, standard-user game-session monitoring, restore/check/quit tray actions, and an in-app remembered close-to-tray or quit dialog.
- Added Flatpak packaging under the current `dev.lockwirelabs.simm` identifier, including containerized Linux build helpers for release testing.
- Added database migration reconciliation and a Settings repair action with a pre-repair backup so installed and development builds can safely recover additive profiles and telemetry tables after historical schema-version mismatches.
- Refined telemetry empty/history states, compacted environment update metadata, and tightened the surrounding desktop layout so status and update actions remain coherent in dense environment rows.
- Stabilized frontend effect dependencies and callback lifecycles across environment, logs, mod-library, mods, plugins, and security-report surfaces, while avoiding repeated telemetry-preference database reads during background monitoring.
- Contributors:
  - `SirTidez`: responsive CSS cleanup for wizard/setup cards, path previews, branch/install selection rows, and flexible grid sizing that keeps long content readable without clipping; concurrent request coalescing across mod-library, environment, and settings refresh paths; short-lived backend mod-library caching; update-check coordination extraction; frontend boot timing instrumentation; profile sharing/import reliability and auto-resolution, managed import entry detection, and target-file matching; profile-sharing review follow-up fixes; first-class Profiles sidebar workspace, runtime-locked profile library defaults, create/capture/import/export/apply workflows, exact-sync profile switching, active-profile usage visibility, and safer in-use profile deletion behavior; 0.8.6 version metadata alignment and pre-publish validation gates; ignored Rust target output in the Vite watcher; async startup flow and boot UI progress handling; Linux parity groundwork with cross-platform readiness and build-tooling updates; Steam auth/UI launch workflow polishing and Steam shortcut reload marker handling; and Logs viewer fixes for multi-line Il2Cpp parsing, severity preservation, installed-mod attribution, live-bottom scrolling, and large stack-trace virtualization. Timestamps: `2026-06-18 19:14:38 PDT`, `2026-06-18 01:11:42 PDT`, `2026-06-14 23:43:31 PDT`, `2026-06-14 23:37:45 PDT`, `2026-06-14 23:29:45 PDT`, `2026-06-14 23:17:13 PDT`, `2026-06-14 23:12:21 PDT`, `2026-06-14 22:05:53 PDT`, `2026-06-14 21:31:02 PDT`, `2026-06-14 12:25:39 PDT`, `2026-06-14 11:25:29 PDT`, `2026-06-13 12:52:28 PDT`, `2026-06-12 00:04:05 PDT`, `2026-06-09 18:44:17 PDT`, `2026-05-21 19:31:18 PDT`, `2026-05-21 11:07:00 PDT`
  - `SirTidez`: opt-in live telemetry, tray-resident runtime controls, Flatpak packaging, database repair/migration reconciliation, compact environment update/status presentation, and frontend lifecycle/performance stabilization. Timestamps: `2026-07-13 19:31:47 PDT`, `2026-07-13 18:12:32 PDT`
  - `ifBars`: Nexus multi-file preservation and file-ID-aware install dedupe so distinct files remain selectable, Steam-shortcut routing for custom installs, preserved launch-error reporting with the inline Steam retry flow, added environment profile sharing, and wizard spacing/dialog color polish for clearer setup screens. Timestamps: `2026-05-31 02:49:40 PDT`, `2026-05-21 18:33:10 PDT`, `2026-05-21 18:38:40 PDT`, `2026-05-21 18:46:51 PDT`
...
- Added project ESLint configuration, fixed mod source links and mod ownership checks, and temporarily disabled featured downloads until package data finishes loading.
...
- Extended archive hardening so unsafe RAR entries are rejected before extraction.
...
- Centralized auth arguments and surfaced backend auth errors more clearly.
- Contributors:
  - `SirTidez`: auth-argument cleanup and clearer backend auth errors. Timestamps: `2026-04-17 12:50:23 PDT`, `2026-04-17 13:42:53 PDT`

## [0.8.1]

- Migrated app updates to the Tauri updater, defaulted update checks to the beta channel, and tightened release validation/dev-startup behavior.
...
- Locked frontend dependencies with a committed `package-lock.json` so local validation uses a reproducible npm toolchain.
- Contributors:
  - `SirTidez`: runtime-aware mod storage/install hardening, installed-environment reporting, `.rar` support, multi-file manual mod uploads, Ko-fi funding metadata, lockfile stabilization, and the `0.8.0` release/versioning pass. Timestamps: `2026-03-30 20:51:25 PDT`, `2026-03-30 21:24:33 PDT`, `2026-03-31 21:16:31 PDT`, `2026-04-02 16:47:37 PDT`, `2026-04-03 08:08:53 PDT`, `2026-04-03 08:27:21 PDT`, `2026-04-03 08:51:33 PDT`
