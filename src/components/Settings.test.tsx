...
import { Settings, normalizeDatabaseBackupCount, normalizeModIconCacheLimitMb } from './Settings';
...
describe('Settings', () => {
  beforeEach(() => {
    ...
    apiMocks.createDirectory.mockResolvedValue({ success: true, path: 'C:\\Games\\New Folder' });
    apiMocks.backupDatabase.mockResolvedValue({ success: true, path: 'C:\\Users\\SirTidez\\SIMM\\backups\\SIMM-db-backup-manual-20260326-034426.db' });
  ...
  it('opens the directory picker from the sidebar and browses the current path', async () => {
    ...
    expect(await screen.findByRole('heading', { name: /select download directory/i })).toBeTruthy();
    await waitFor(() => {
  ...
  it('creates a folder from the directory picker and refreshes the listing', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/create a folder in the current location/i), {
      target: { value: 'New Folder' },
    });
    fireEvent.click(screen.getByRole('button', { name: /create folder/i }));

    await waitFor(() => {
      expect(apiMocks.createDirectory).toHaveBeenCalledWith('C:\\Games\\New Folder');
    });
    await waitFor(() => {
  ...
  it('only exposes built-in theme presets', async () => {
    ...
    const themeField = screen.getByText(/theme preset/i).closest('.settings-field');
    const select = themeField?.querySelector('select') as HTMLSelectElement | null;
  ...
  it('opens the backups folder from settings', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /open backups folder/i }));

    await waitFor(() => {
      expect(apiMocks.getHomeDirectory).toHaveBeenCalledTimes(1);
    });
    await waitFor(() => {
      expect(apiMocks.openPath).toHaveBeenCalledWith('C:\\Users\\SirTidez\\SIMM\\backups');
    });
  });
});
